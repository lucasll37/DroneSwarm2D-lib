...
- Coordenadas contínuas (`pos`): posição real dos drones em pixels
- Coordenadas discretas (`cell`): células da grade para detecção
...
# o alvo é considerado detectado mesmo sem detecção direta
```
...
Para simulações grandes:
- Reduza `CICLE_COMM_BY_STEP` para diminuir custo de comunicação
- Aumente `CELL_SIZE` para reduzir resolução das matrizes
- Desative `PASSIVE_DETECTION` se não for necessário
- Use `fullscreen=False` e reduza resolução da janela
...
```python
# No config.json
"PASSIVE_DETECTION": true  # Mostrar linhas de visada
```
...
**Desenvolvido como parte do Trabalho de Graduação do ITA**
